...
from logging import DEBUG, Logger  # noqa
from socket import getdefaulttimeout as get_default_timeout
from typing import Container, Dict, List, Optional, Text, Tuple, Union  # noqa

from requests import Response, codes, request, auth  # noqa
...
if PY2:
    ...
    from urlparse import SplitResult  # noqa
else:
    # noinspection PyCompatibility,PyUnresolvedReferences
    from urllib.parse import SplitResult  # noqa
...
class HttpAdapter(BaseAdapter):
    ...
    def _interpret_response(self, response, payload, expected_status):
        ...
        if not isinstance(decoded, dict):
            raise with_context(
                exc=BadApiResponse(
                    'Malformed {status} response from node: '
                    '{decoded!r}'.format(
                        status=response.status_code,
                        decoded=decoded,
                    ),
...
class MockAdapter(BaseAdapter):
    ...
    def send_request(self, payload, **kwargs):
        ...
        if error:
            raise with_context(
                BadApiResponse(error), context={'request': payload})
