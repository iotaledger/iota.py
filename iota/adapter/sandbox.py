...
from typing import Container, Optional, Text, Union  # noqa

from requests import Response, codes  # noqa
from six import moves as compat, text_type

from iota.adapter import BadApiResponse, HttpAdapter, SplitResult  # noqa
        ...
        if response.status_code == codes['accepted']:
            ...
            while decoded['status'] in (STATUS_QUEUED, STATUS_RUNNING):
                if poll_count >= self.max_polls:
                    raise with_context(
                        exc=BadApiResponse(
                            '``{command}`` job timed out after {duration} '
                            'seconds (``exc.context`` has more info).'.format(
            ...
            if decoded['status'] == STATUS_FINISHED:
                return decoded['{command}Response'.format(
                    command=decoded['command'])]
