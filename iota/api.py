...
class StrictIota(with_metaclass(ApiMeta)):
        ...
        try:
            ...
            raise InvalidCommand(
                '{cls} does not support {command!r} command.'.format(
                    cls=type(self).__name__, command=command,),
            )
    ...
    def check_consistency(self, tails):
        ...
        """
        Used to ensure tail resolves to a consistent ledger which is necessary
        to validate before attempting promotionChecks transaction hashes for
        promotability.
        ...
        transactions simultaneously if you give more than one) to be confirmed,
        or not (because it conflicts with another already confirmed
        transaction).
        ...
        """
        return core.CheckConsistencyCommand(self.adapter)(tails=tails,)
...
class Iota(StrictIota):
    ...
    def get_bundles(self, transaction):
        ...
        """  # noqa
        return extended.GetBundlesCommand(self.adapter)(
    ...
    def is_reattachable(self, addresses):
        ...
        This API function helps you to determine whether you should replay a
         transaction or make a completely new transaction with a different
         seed. What this function does, is it takes one or more input
         addresses (i.e. from spent transactions) as input and then checks
         whether any transactions with a value transferred are confirmed.
         If yes, it means that this input address has already been successfully
         used in a different transaction and as such you should no longer
