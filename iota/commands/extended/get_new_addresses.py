        ...
        return {
            'addresses':
                self._find_addresses(
                    seed, index, count, security_level, checksum),
        }
        ...
        super(GetNewAddressesRequestFilter, self).__init__(
            {
                ...
                'index':
                    flt.Type(int)
                    | flt.Min(0)
                    | flt.Optional(default=0),
                'securityLevel':
                    flt.Type(int)
                    | flt.Min(1)
                    | flt.Max(self.MAX_SECURITY_LEVEL)
                    | flt.Optional(
                        default=AddressGenerator.DEFAULT_SECURITY_LEVEL),
                'seed': flt.Required | Trytes(result_type=Seed),
            },
            allow_missing_keys={
                'checksum',
                'count',
