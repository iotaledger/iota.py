...
from typing import Generator, Iterable, List, Tuple  # noqa
...
def iter_used_addresses(adapter, seed, start):
    # type: (BaseAdapter, Seed, int)
    #    -> Generator[Tuple[Address, List[TransactionHash]]]
    """
...
def get_bundles_from_transaction_hashes(
    ...
    all_transactions = list(map(
        Transaction.from_tryte_string,
        gt_response['trytes']))  # type: List[Transaction]
    ...
    tail_transactions = [txn for txn in all_transactions
