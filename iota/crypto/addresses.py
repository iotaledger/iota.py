...
from typing import Generator, Iterable, List, MutableSequence  # noqa

from iota import Address, TRITS_PER_TRYTE, TrytesCompatible  # noqa
from iota.crypto.kerl import Kerl
from iota.crypto.signing import KeyGenerator, KeyIterator # noqa
from iota.crypto.types import Digest, PrivateKey, Seed  # noqa
...
class AddressGenerator(Iterable[Address]):
    ...
    def create_iterator(self, start=0, step=1):
        ...
        key_iterator = (
            KeyGenerator(self.seed).create_iterator(
                start, step, self.security_level)
        )
    ...
    def _generate_address(self, key_iterator):
        ...
        """
        if self.checksum:
            return self.address_from_digest(
                self._get_digest(key_iterator)).with_valid_checksum()
        else:
