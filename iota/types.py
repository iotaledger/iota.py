...
class TryteString(JsonSerializable):
    ...
    def from_bytes(cls,  # Type[T]
                   bytes_,  # Union[binary_type, bytearray]
    ...
    def from_string(cls, *args, **kwargs):
        ...
        """
        warn(
            message='`from_string()` is deprecated; '
                    'use `from_unicode()` instead.',
        ...
        if isinstance(trytes, TryteString):
            if type(trytes) is TryteString or issubclass(
              type(trytes), type(self)):
                ...
                raise with_context(
                    exc=TypeError(
                        '{cls} cannot be initialized from a(n) {type}.'
                        .format(
                            type=type(trytes).__name__,
                            cls=type(self).__name__,
                        )),
    ...
    def decode(self, errors='strict', strip_padding=True):
        ...
        return decode(trytes, AsciiTrytesCodec.name, errors)\
            .decode('utf-8', errors)
    ...
    def as_integers(self):
        ...
        """
        return [self._normalize(AsciiTrytesCodec.index[c])
    ...
    def _normalize(n):
        # type: (int) -> int
        if n > 26:
            raise ValueError('{n} cannot be represented by a single tryte.'
                             .format(n=n))
...
class Hash(TryteString):
    ...
    def __init__(self, trytes):
        ...
        if len(self._trytes) > self.LEN:
            raise with_context(
                exc=ValueError(
                    '{cls} values must be {len} trytes long.'
                    .format(
                        cls=type(self).__name__,
                        len=self.LEN
                    )),
...
class Address(TryteString):
    ...
    def __init__(self,  # TrytesCompatible
                 trytes,  # Optional[int]
        ...
        if len(self._trytes) == (self.LEN + AddressChecksum.LEN):
            self.checksum = AddressChecksum(self[self.LEN:])

        elif len(self._trytes) > self.LEN:
            raise with_context(
                exc=ValueError(
                    'Address values must be {len_no_checksum} trytes'
                    ' (no checksum), or {len_with_checksum} trytes'
                    ' (with checksum).'
                    .format(
...
class AddressChecksum(TryteString):
    ...
    def __init__(self, trytes):
        ...
        if len(self._trytes) != self.LEN:
            raise with_context(
                exc=ValueError(
                    '{cls} values must be exactly {len} trytes long.'
                    .format(
                        cls=type(self).__name__,
                        len=self.LEN,
                    )),
